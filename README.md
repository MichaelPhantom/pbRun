...
- ✅ **AI 教练分析** - 活动详情页基于本次配速/心率/步频/VDOT 与每公里分段，
  调用本机 freellm 网关生成中文解读与训练建议（SSE 流式，模型可选，默认 auto；
  推荐非思考模型如 glm-5.1-wb 以避免输出截断，凭证见 `.env` 的 `FREELLMAPI_KEY`）
...
- 访问: 经 u2 的 Nginx 门户反代 `http://hostip/pbrun/` (Basic Auth 保护)
